...
const getRoute = async (
  ...
  try {
    console.log(
      `🔍 Checking arbitrage: ${mintAddr1.slice(0, 4)}...${mintAddr1.slice(
        -4
      )} -> ${mintAddr2.slice(0, 4)}...${mintAddr2.slice(
        -4
      )} -> ${mintAddr1.slice(0, 4)}...${mintAddr1.slice(-4)}`
    );
    ...
    try {
      ...
      console.log(
        `🛣️ Trying multi-hop route through ${mintAddr2.slice(0, 8)}...`
      );
      ...
      if (multiHopRes.ok) {
        ...
        console.log(
          `✅ Got multi-hop quote: ${
            arbitrageQuote.routePlan?.length || 0
          } steps`
        );
      } else {
      ...
      try {
        ...
        const quote1Url = `${SWAP_QUOTE_BASE_URL}?inputMint=${mintAddr1}&outputMint=${mintAddr2}&amount=${amountIn}&slippageBps=100`;
        const res1 = await fetch(quote1Url);
        if (!res1.ok) throw new Error(`Quote1 failed: ${res1.status}`);
        const quote1 = (await res1.json()) as any;

        const quote2Url = `${SWAP_QUOTE_BASE_URL}?inputMint=${mintAddr2}&outputMint=${mintAddr1}&amount=${quote1.outAmount}&slippageBps=100`;
        ...
        console.log(
          `   Step 1: ${amountIn} WSOL -> ${quote1.outAmount} ${mintAddr2.slice(
            0,
            8
          )}`
        );
        console.log(
          `   Step 2: ${quote1.outAmount} ${mintAddr2.slice(0, 8)} -> ${
            quote2.outAmount
          } WSOL`
        );
        console.log(
        ...
        const minimumProfitRequired =
          amountIn + upperAmountWithDecimal + estimatedGasCost + safetyMargin;

        if (minimumProfitRequired > Number(quote2.outAmount)) {
          const actualLoss = minimumProfitRequired - Number(quote2.outAmount);
          console.log(
            `💸 Not profitable after fees - IN: ${amountIn}, OUT: ${Number(
              quote2.outAmount
            )}, Loss: ${actualLoss}`
          );
          ...
          totalTrades++;
          console.log(
            `📊 SUCCESS RATE: ${successfulTrades}/${totalTrades} (${(
        ...
        console.log(
          `📈 NET PROFIT (after fees): ${netProfitSol.toFixed(
            6
          )} SOL (${netProfitPct.toFixed(3)}%)`
        );
        ...
        await sleep(500); // Increased delay for better reliability

        console.log("🚀 Step 2: Execute second swap...");

        // Get the actual balance of intermediate token
        const intermediateBalance = await connection.getTokenAccountBalance(
          intermediateATA
        );
        const actualIntermediateAmount = intermediateBalance.value.amount;

        console.log(
          `💰 Intermediate token balance: ${actualIntermediateAmount}`
        );

        // Verify we have enough intermediate tokens
        if (Number(actualIntermediateAmount) === 0) {
          console.log(
            "❌ No intermediate tokens found - first swap may have failed"
          );
          totalTrades++;
          console.log(
            `📊 SUCCESS RATE: ${successfulTrades}/${totalTrades} (${(
        ...
        if (!res2Fresh.ok)
          throw new Error(`Fresh quote2 failed: ${res2Fresh.status}`);
        const quote2Fresh = (await res2Fresh.json()) as any;

        console.log(
          `🔄 Fresh quote2: ${actualIntermediateAmount} ${mintAddr2.slice(
            0,
            8
          )} -> ${quote2Fresh.outAmount} WSOL`
        );
        ...
        const actualProfit =
          (afterBalance.value.uiAmount || 0) -
          (beforeBalance.value.uiAmount || 0);
        const totalChange = initialBalance - (afterBalance.value.uiAmount || 0);

        console.log(
          `💰 Balance: ${beforeBalance.value.uiAmount} -> ${afterBalance.value.uiAmount}`
        );
        console.log(
          `💹 ACTUAL PROFIT: ${actualProfit.toFixed(6)} SOL ($${(
            actualProfit * 200
          ).toFixed(4)})`
        ); // Assuming $200/SOL
        console.log(
          `📊 Total P&L: ${totalChange > 0 ? "+" : ""}${totalChange}`
        );
        ...
        console.log(
          `💰 TOTAL PROFIT: ${totalProfit.toFixed(6)} SOL ($${(
            totalProfit * 200
          ).toFixed(2)})`
        );
    ...
    if (amountIn + upperAmountWithDecimal > Number(arbitrageQuote.outAmount)) {
      console.log(
        ...
        amountIn,
        "=> OUT:",
        Number(arbitrageQuote.outAmount)
      );
      totalTrades++;
      console.log(
        `📊 SUCCESS RATE: ${successfulTrades}/${totalTrades} (${(
    ...
    console.log(
      `📈 PROFIT: ${profitSol.toFixed(6)} SOL (${profitPct.toFixed(2)}%)`
    );
    console.log(
      "📈 IN:",
      amountIn,
...
const start = async () => {
  ...
  const stables = [
    ...
    // HIGH VOLUME MEME COINS (Best arbitrage opportunities)
    "DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263", // BONK - Highest volume Solana meme
    "EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm", // WIF - Top dog-themed meme
  ...
  while (1) {
    ...
    if (totalTrades > 0) {
      console.log(
        `\n📊 CYCLE STATS: ${successfulTrades}/${totalTrades} success, ${totalProfit.toFixed(
          6
        )} SOL profit\n`
      );
    }
